...
func writeTOML(path string, mappings []AppMapping) error {
	// Categorize function for metadata
	...
	// Build data structures for TOML encoding
	aliases := make(map[string]string)
	metadata := make(map[string]string) // app -> category
	...
	for _, m := range mappings {
		...
		category := categorize(m.IconName)
		...
		for _, appName := range m.AppNames {
			...
			aliases[normalizedApp] = targetIcon
			metadata[normalizedApp] = category
		}
	}
	...
	for _, m := range mappings {
		...
		if m.GlyphChar != "" && len(m.GlyphChar) > 0 {
			r := []rune(m.GlyphChar)[0]
			symbols[targetIcon] = fmt.Sprintf("\\U%08X", r)
		}
	}

	// Create output structure - toml library sorts map keys alphabetically
	...
	fmt.Fprintf(f, "#\n")
	fmt.Fprintf(f, "# [aliases]  - Maps app names to icon names\n")
	fmt.Fprintf(f, "# [symbols]  - Maps icon names to Nerd Font glyphs\n")
